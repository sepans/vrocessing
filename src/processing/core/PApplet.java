...
//    println("init() called " + Integer.toHexString(hashCode()));
    ...
    if ((size.width != 0) && (size.height != 0)) {
      ...
      System.out.println("make graphics");
    ...
    if (g != null && (looping || redraw)) {
      ...
      if (frameCount == 0) {
        ...
        if(Math.random()<0.01) {
            ...
            int displace = (int) Math.round(Math.random()*width+width*0.05);
            //println("sq size: "+displace);
            ...
            for(int i=0; i<width;i++) {
              for(int j=0; j<height;j++) {
                if(Math.random()<0.5)
                  pixels[i*height+j]=pixels[(i*height+j+displace)%(width*height)];
                else
                  pixels[(j*height+j)%(width*height)]=pixels[(j*height+i+displace)%(width*height)];
