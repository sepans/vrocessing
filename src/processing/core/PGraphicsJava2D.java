...
import processing.core.stochastic.Deformation;
import processing.core.stochastic.Functions;
import processing.core.stochastic.StochasticEngine;
...
public class PGraphicsJava2D extends PGraphics /*PGraphics2D*/ {
  ...
  StochasticEngine stEngine =StochasticEngine.getReference();
...
//      }
  ...
  //removed by sepans: to make PGraphics.line run
  
  int lineCount =0;

  public void line(float x1, float y1, float x2, float y2) {

    Deformation def = stEngine.getDeformation(Functions.line.toString()+lineCount);
    
    if(def!=null) {
        if(def.role.equals("break")) {
         // int middlePointsNum = Integer.parseInt(def.params.get("pointCount").toString());
 

          float stepX = Float.parseFloat(def.params.get("stepX").toString());
          float stepY = Float.parseFloat(def.params.get("stepY").toString());
          //PVector[] middlePoints = new PVector[middlePointsNum];
          ...
          while(true) {
           // float xProgress = (float) Math.random();
           // float yProgress = (float) Math.random();
            
            float x= prevX + (x2-x1)*stepX;
            float y= prevY + (y2-y1)*stepY;
            ...
            prevX = x;
            prevY = y;
            //System.out.println("middle "+middlePoints[i].x+" y: "+middlePoints[i].y+" ");
          }
          line.setLine(prevX, prevY, x2, y2);
          strokeShape(line);
        }
        if(def.role.equals("box")) {
          if(Math.random()<0.6)
            noFill();
          float sizeX = Float.parseFloat(def.params.get("sizeX").toString());
          float sizeY = Float.parseFloat(def.params.get("sizeY").toString());
          rect(x1,y1,sizeX,sizeY);
        }
        if(def.role.equals("cir")) {
          //if(Math.random()<0.8)
          //  noFill();
          float sizeX = Float.parseFloat(def.params.get("sizeX").toString());
          float sizeY = Float.parseFloat(def.params.get("sizeX").toString());
          ellipse(x1,y1,sizeX,sizeY);
        }
        if(def.role.equals("mul")) {
          //System.out.println("number "+def.params.get("num").toString());
          int num = Integer.parseInt(def.params.get("num").toString());
          for(int i=0;i<num;i++) {
            line.setLine(x1+(i*10),y1,x2,y2+(i*10));
            strokeShape(line);
         }
        }
         if(def.role.equals("disp")) {
            //System.out.println("number "+def.params.get("num").toString());
            float dispX1 = Float.parseFloat(def.params.get("dispX1").toString());
            float dispX2 = Float.parseFloat(def.params.get("dispX2").toString());
            float dispY1 = Float.parseFloat(def.params.get("dispY1").toString());
            float dispY2 = Float.parseFloat(def.params.get("dispY2").toString());
            line.setLine(x1+dispX1,y1+dispY1,x2+dispX2,y2+dispY2);
            strokeShape(line);
          }
         if(def.role.equals("blck")) {
           //if(Math.random()<0.8)
           //  noFill();
           int c= color(Integer.parseInt(def.params.get("colorR").toString()),
                        Integer.parseInt(def.params.get("colorG").toString()),
                        Integer.parseInt(def.params.get("colorB").toString()));
           fill(c);
           rect(0,0,width,height);
         }
         if(def.role.equals("rst")) {
           //if(Math.random()<0.8)
           //  noFill();
    ...
    lineCount ++;
   /* if(!this.stochastic || Math.random()>0.3) {
      line.setLine(x1, y1, x2, y2);
      strokeShape(line);
    }
    else {
     // System.out.println("random x1:" + x1 + " y1:"+ y1 + " x2: "+ x2 + " y2: "+ y2 + " ");
      if(Math.random()<0.6) {
             int displaceX = (int) Math.round(Math.random()*width*0.1);
             int displaceY = (int) Math.round(Math.random()*height*0.1);
             x1=(x1+displaceX)%width;
             y2=(y2+displaceY)%height;
      }
      
      
      
      if(Math.random()<0.005) {
        int displace = (int) Math.round(Math.random()*width+width*0.1);
        line.setLine((x2+displace)%width, x1%height, y2%width, y1%height);
        strokeShape(line);
        
      }
      if(Math.random()<0.005) {
        int displace = (int) Math.round(Math.random()*width+width*0.1);
        line.setLine(y2%width, (y1+displace)%height, x2%width, x1%height);
        strokeShape(line);
        
      }
      else if(Math.random()<0.2) {
        int middlePointsNum = (int) Math.round(Math.random()*2+1);
        PVector[] middlePoints = new PVector[middlePointsNum];

        
        for(int i=0;i<middlePointsNum;i++) {
          middlePoints[i]= new PVector();
          float xProgress = (float) Math.random();
          float yProgress = (float) Math.random();
          middlePoints[i].x= x1 + (x2-x1)*xProgress;
          middlePoints[i].y= y1 + (y2-y1)*yProgress;
          //System.out.println("middle "+middlePoints[i].x+" y: "+middlePoints[i].y+" ");
          if(i==0) {
            line.setLine(x1, y1, middlePoints[i].x, middlePoints[i].y);
            strokeShape(line);
          }
          else if(i==middlePointsNum) {

            line.setLine(middlePoints[i].x, middlePoints[i].y, x2, y2);
            strokeShape(line);
          }
          else {
            line.setLine(middlePoints[i-1].x, middlePoints[i-1].y, middlePoints[i].x, middlePoints[i].y);
            strokeShape(line);
